...
window.onload = () => {
    ...
    const groupCountBadge = document.getElementById("group-count-badge");
    let currentGroups = [];
    let sortTimeoutId;
    ...
    function renderGroups(groups) {
        ...
        const cards = Array.from(resultContainer.children);
    ...
    function animateGroupsToSorted(initialGroups) {
        // measure current positions
        const firstRects = new Map();
        const oldCards = Array.from(resultContainer.children);
        ...
        const newCards = Array.from(resultContainer.children);
        ...
        setTimeout(() => {
            ...
            updateGroupBadge();
            const cardsNow = Array.from(resultContainer.children);
            cardsNow.forEach((card) => {
                ...
                if (!group)
                    return;
                const wordsWrap = card.querySelector(".group-words");
                if (!wordsWrap)
                    return;
                wordsWrap.innerHTML = "";
                group.words.forEach((word) => {
    ...
    sortButton.addEventListener("click", () => {
        ...
        sortedWords = anagrams.reduce((acc, word, index) => {
